...
from datasets.tu_dataset import TUDataset
...
from torch_geometric.data import DenseDataLoader as DenseLoader,DataLoader


def cross_validation_with_val_set(dataset,model,collate,folds,epochs,batch_size,lr,factor,weight_decay,device,log=None,seed=234):
    """Cross validation framework. Adapted from Nested GNN:https://github.com/muhanzhang/NestedGNN
    Args:
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(
        ...
        if 'adj' in train_dataset[0]:
            ...
            val_loader = DenseLoader(val_dataset, batch_size, shuffle=False,collate_fn=collate)
        ...
        for epoch in pbar:
            ...
            cur_accs.append(train_utils.test_TU(model, test_loader, device))
            eval_info = {
                'fold': fold,
                'epoch': epoch,
            ...
            info = 'Fold: %d, train_loss: %0.4f, val_loss: %0.4f, test_acc: %0.4f' % (
        ...
        loss, argmin = torch.tensor(cur_val_losses).min(dim=0)
        acc = cur_accs[argmin.item()]
        final_train_losses.append(eval_info["train_loss"])
        info = 'Fold: %d, final train_loss: %0.4f, best val_loss: %0.4f, test_acc: %0.4f' % (
    ...
    info = 'Val Loss: {:.4f}, Test Accuracy: {:.3f} ± {:.3f}, Duration: {:.3f}'.format(
        loss.mean().item(),
        acc.mean().item(),
        acc.std().item(),
        duration.mean().item()
...
def cross_validation_without_val_set(dataset,model,collate,folds,epochs,batch_size,lr,factor,weight_decay,device,log=None,seed=234):
...
def main():
    ...
    parser.add_argument('--dataset_name',type=str,default="MUTAG",choices=('DD', 'MUTAG', 'PROTEINS', 'PTC_MR', 'ENZYMES'),help='name of dataset')
    ...
    parser.add_argument("--hidden_size",type=int,default=66,help="hidden size of the model")
    ...
    parser.add_argument("--JK",type=str,default="attention",choices=("sum","max","mean","attention","last","concat"),help="Jumping knowledge method")
    ...
    parser.add_argument('--aggr',type=str,default="mean",help='aggregation method in GNN layer, only works in GraphSAGE')
    ...
    dataset = TUDataset(path, args.dataset_name, pre_transform=T.Compose([convert_node_and_edge_labels,pre_transform,rd_feature]), cleaned=args.clean)
    args.input_size=dataset.num_node_labels
    args.num_hop1_edge=dataset.num_edge_labels+1
    ...
    loss,acc,std=cross_val_method(dataset,model,collate=PyG_collate,folds=args.split,epochs=args.num_epochs,
